...
namespace PikachuClassic
{
    ...
    public class Grid
    {
        ...
        public void GenerateGrid()
        {
            ...
            for (int i = 0; i < rows; i++) //định sửa: for (int i = 1; i < rows - 1; i++)
            {
                for (int j = 0; j < cols; j++) //định sửa: for (int j = 1; j < cols - 1; j++)
                {
        ...
        {
            int totalCells = rows * cols; //lúc trước: rows - 2, cols - 2
            if (totalCells % 2 != 0)
            {
            ...
            int imageIndex = 0;
            for (int i = 0; i < rows; i++) //lúc trước: int i = 1; i < rows - 1; i++
            {
                for (int j = 0; j < cols; j++) //lúc trước: int j = 1; j < cols - 1; j++
                {
        ...
        // Lấy tọa độ trung tâm của PictureBox
